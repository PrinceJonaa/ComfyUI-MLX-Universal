...
class TestGenerateNodes(unittest.TestCase):
    ...
    @patch.object(import_node_module("generate_nodes"), "execute_batch_image_description")
    def test_mlx_vlm_batch_run_happy_path(self, mock_execute):
        node = self.MLXVLMBatchDescribeImage()
        mocked_model = self.get_mocked_model()
        mocked_model.family = "mlx-vlm"

        mock_execute.return_value = "batch images described"
        ...
        self.assertEqual(result, ("batch images described",))
        mock_execute.assert_called_once_with(
            mlx_model=mocked_model,
            prompt="Describe this batch",
            max_tokens=256,
