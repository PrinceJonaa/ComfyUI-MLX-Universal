...
class TestGenerateNodes(unittest.TestCase):
    ...
    def test_mlx_lm_generate_unknown_model_family_raises_value_error(self):
        ...
        self.assertEqual(
            ...
            "Expected model family 'mlx-lm' but found 'unknown-family'. Ensure you passed a text model to 'MLX Load Model' (e.g. Qwen, Llama). If passing a Vision/Audio model, use the appropriate node.",
        )
    ...
    def test_mlx_vlm_run_unknown_model_family_raises_value_error(self):
        ...
        self.assertEqual(
            str(context.exception),
            "Expected model family 'mlx-vlm' but found 'unknown-family'. Ensure you passed a Vision-Language Model to 'MLX Load Model'. Check the supported models list if unsure.",
        )
