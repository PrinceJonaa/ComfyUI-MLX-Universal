...
def _split_qkv_proj(state_dict, key_substring="attn.qkv"):
    import mlx.core as mx
    ...
    for k in state_dict:
        if key_substring in k:
            ...
            for name, weight in zip(["q", "k", "v"], mx.split(state_dict[k], 3)):
                state_dict_update[k.replace(key_substring, f"attn.{name}_proj")] = (
...
def flux_state_dict_adjustments(state_dict, prefix="", hidden_size=3072, mlp_ratio=4):
    ...
    state_dict = _split_qkv_proj(state_dict, "attn.qkv")
...
def mmdit_state_dict_adjustments(state_dict, prefix=""):
    ...
    new_state_dict = {}
    for k, v in state_dict.items():
        if "decoder." in k or "k_proj.bias" in k or "teacher_model." in k:
            continue

        new_k = k.replace("attn.proj", "attn.o_proj")

        # pos_embed logic
        if "pos_embed" in new_k:
            new_k = new_k.replace("pos_embed", "x_pos_embedder.pos_embed.weight")
            new_state_dict[new_k] = v[0]
        else:
            new_state_dict[new_k] = v

    new_state_dict["x_embedder.proj.weight"] = new_state_dict[
...
def _common_vae_adjustments(state_dict):
    ...
    new_state_dict = {}
    for k, v in state_dict.items():
        new_k = k.replace("mid", "mid_blocks")
        for old, new in replacements:
            if old in new_k:
                new_k = new_k.replace(old, new)
        new_state_dict[new_k] = v
    ...
    if "conv_in.weight" in new_state_dict:
        new_state_dict["conv_in.weight"] = new_state_dict["conv_in.weight"].transpose(
            0, 2, 3, 1
        )
    if "conv_out.weight" in new_state_dict:
        new_state_dict["conv_out.weight"] = new_state_dict["conv_out.weight"].transpose(
            0, 2, 3, 1
        )
