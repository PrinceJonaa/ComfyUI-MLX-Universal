...
This PR implements an explicit batch processing path for VLM image understanding to fulfill the feature expansion requirement.

## What Changed
*   **Runtime:** Added `execute_batch_image_description` in `runtime/generate_processing.py`. This function converts a batched ComfyUI `IMAGE` tensor into a list of PIL images and iteratively queries the VLM via `mlx_vlm.generate`. Crucially, it forces evaluation (`mx.eval`) and clears the Metal cache (`mx.metal.clear_cache()`) per iteration to prevent Apple Silicon Unified Memory from crashing due to massive lazy graph accumulation.
