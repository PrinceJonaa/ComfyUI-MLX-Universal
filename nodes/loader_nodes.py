            ...
            "required": {
                "model_path": (
                    "STRING",
                    {
                        ...
                        "tooltip": "Hugging Face repository ID or local path for the model.",
                    },
                ),
                "model_type": (
                    ...
                    {
                        "default": "auto",
                        "tooltip": "The family of the model. Use auto to auto-detect.",
                    },
                ),
                "trust_remote_code": (
                    "BOOLEAN",
...
class MLXDraftModelLoader:
            ...
            "required": {
                "model_path": (
                    "STRING",
                    {
                        ...
                        "tooltip": "Hugging Face repository ID or local path for the draft model.",
                    },
                ),
                "model_family": (
                    ["mlx-lm", "mlx-vlm"],
                    {"default": "mlx-lm", "tooltip": "The family of the draft model."},
                ),
            }
        }
    ...
    def load_model(self, model_path: str, model_family: str) -> tuple:
        if not model_path:
            raise ValueError(
                "Expected a valid Hugging Face repository ID or local path, but found an empty draft model path. Please provide a valid model path for the draft model."
            )
