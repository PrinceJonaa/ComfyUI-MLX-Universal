...
def get_model_config(model_path_or_id):
    if os.path.exists(os.path.join(model_path_or_id, "config.json")):
        ...
        try:
            config_path = hf_hub_download(repo_id=model_path_or_id, filename="config.json")
        except Exception:
            return {}
    try:
...
def detect_model_type(model_path_or_id):
    ...
    if not config:
        ...
        if "sam3" in name_lower: return "sam3"
        if any(v in name_lower for v in ["vlm", "vl-", "llava", "gemma-3n", "gemma-4"]): return "mlx-vlm"
        return "mlx-lm"
    model_type = config.get("model_type", "").lower()
    architectures = [a.lower() for a in config.get("architectures", [])]
    if "sam3" in model_type or any("sam3" in arch for arch in architectures) or "sam3" in model_path_or_id.lower():
        return "sam3"
    vlm_indicators = ["vlm", "vl", "llava", "idefics", "mflux", "paligemma", "clip", "siglip", "gemma_3n", "gemma_4"]
    is_vlm = any(ind in model_type for ind in vlm_indicators) or any(any(ind in arch for ind in vlm_indicators) for arch in architectures) or "vision_config" in config
    if is_vlm: return "mlx-vlm"
    return "mlx-lm"
...
class MLXModelLoaderUnified:
    ...
    def load_model(self, model_path, model_type, trust_remote_code, quantize_activations, adapter_path=""):
        resolved_type = detect_model_type(model_path) if model_type == "auto" else model_type
        print(f"Automatically detected model type for '{model_path}': {resolved_type}" if model_type == "auto" else f"Using specified model type: {resolved_type}")
        
        key_suffix = f":adapter={adapter_path}" if adapter_path else ""
        cache_key = make_key(model_path, resolved_type, trust_remote_code, quantize_activations) + key_suffix
        ...
        def _load():
            ...
            if resolved_type == "mlx-lm":
                ...
                return LoadedMLXModel(family=resolved_type, model_path=model_path, model_type=resolved_type, trust_remote_code=trust_remote_code, quantize_activations=quantize_activations, model=model, processor=processor)
